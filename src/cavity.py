...
from state_monitor import connection_cb, rf_on_cb
...
class Cavity:
    def __init__(self, name: str, epics_name: str, cavity_type: str, length: float,
        ...
        if self.cavity_type == "C100" or self.cavity_type == "C75":
            # 1 = RF on, 0 = RF off
            self.rf_on = epics.PV(f"{self.epics_name}RFONr", connection_callback=connection_cb)
            self.gset_min = 5
        elif self.cavity_type == "C50" or self.cavity_type == "C25":
            # 1 = RF on, 0 = RF off
    ...
    def set_gradient(self, gset: float):
        if gset != 0 and self.bypassed:
        ...
        if gset > self.odvh.value:
            msg = f"{self.name}: Can't turn cavity too far above operational max"
            logger.error(msg)
            raise ValueError(msg)
    ...
    def restore_pset(self):
        self.pset.value = self.pset_init
